...
| 项 | 选型 | 说明 |
| --- | --- | --- |
| 语言 | **Java 17**（沿用参考项目随附 `jvm64`） | 与 RustedWarfare 一致，便于复用素材 |
| 渲染 | **Java2D / Swing（`JPanel` + `Graphics2D`）** | demo 阶段轻量，无需 LWJGL；后期可替换为 libGDX |
| 配置格式 | **JSON**（单位/关卡） | 可读、易解析（`org.json` 或 `Jackson`） |
| 美术素材 | 复用 `RustedWarfare/assets/units/*.png` | 见分析文档第七节映射表 |
| 构建 | Maven / 单 `javac` 均可 | demo 可不引入重型构建 |
...
| 单位 | 移动域 | 打地 | 打水面 | 打空 | 打水下 | 角色 |
| --- | --- | :---: | :---: | :---: | :---: | --- |
| 轻型坦克 | LAND | ✓ | ✓ | ✗ | ✗ | STRIKE |
| 重型坦克 | LAND | ✓ | ✓ | ✓ | ✗ | STRIKE |
| 战列舰 | WATER | ✗ | ✓ | ✗ | ✗ | STRIKE |
| 驱逐舰 | WATER | ✗ | ✓ | ✓ | ✓ | STRIKE |
| 潜艇 | UNDERWATER | ✗ | ✓ | ✗ | ✓ | STRIKE |
| 拦截机 | AIR | ✗ | ✗ | ✓ | ✗ | STRIKE |
| 攻击机 | AIR | ✓ | ✓ | ✗ | ✗ | STRIKE |
| 侦察单位 | LAND/AIR | 弱自卫 | — | — | — | SCOUT |
...
```json
{
  ...
  "playerBudget": { "light_tank": 4, "scout": 2, "destroyer": 2 },
  "enemyUnits": [
    { "unitId": "heavy_tank", "col": 100, "row": 60 },
    { "unitId": "submarine",  "col": 110, "row": 90 }
  ]
}
```

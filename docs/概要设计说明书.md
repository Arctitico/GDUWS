...
| `GameRenderer` | 纯绘制：地形→装饰→残骸→网格→单位→血条→射弹→爆炸特效→攻击线→路径→视野→已知敌情→雾 | `render(Graphics2D, World)` | World（只读） |
...
**Unit（运行时）**：每个战场上的单位实例。持有 `def`（指向 UnitDef）、`faction`、`x/y`（像素坐标）、`hp`、`facing`（弧度）、`turretFacing`（炮塔朝向，有炮塔单位独立于底座）、`role`（SCOUT/STRIKE，运行时可变）、`state`（AI 状态）、`path`（`Deque<Point>` 格序列）、`moveGoal`（移动终点格）、`currentTarget`、`shootCooldown`（射击冷却剩余 tick）。
...
以下特性原列于本文档"扩展点与未来方向"，已于后续迭代中实现，对应文档已上移：

| 特性 | 实现版本 | 对应模块 | 文档位置 |
|---|---|---|---|
| 战争迷雾（FR-20） | `6f625b1` | `FogRenderer` | §4.3 表现层、§6.5、§7.5 |
| 射弹实体（FR-21） | `c8494f3` | `ProjectileSystem` / `Projectile` / `ProjectileType` | §4.1 模拟层、§6.4、§7.4 |
| 单位残骸 | `603149d` | `Wreckage` | §4.1 模拟层、§5.2 |
| Scout↔Strike 自动转换 | `699266a` | `AISystem` / `StrikeAI` | §6.6 |
| 空间协调工具 | `2626617` | `UnitSpacing` | §4.1 模拟层、§7.6 |
